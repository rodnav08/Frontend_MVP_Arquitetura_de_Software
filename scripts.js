...
const perfildev = async () => {
  let url = 'http://127.0.0.1:5000/perfildev';
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error('Erro ao exportar o perfil do desenvolvedor:', response.status);
    }
  } catch (error) {
    console.error('Erro ao exportar o perfil do desenvolvedor:', error);
  }
};
